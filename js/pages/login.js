...
export function renderLogin(root) {
      ...
      <div class="scroll" style="padding-bottom:0">
        <div class="login-page">
          ...
          <div style="text-align:center; color:var(--text-faint); font-size:12px; line-height:1.6; max-width:280px">
            Data stored in Firebase, synced across all your devices.<br>
            No password needed — sign in with your account.
          </div>
        </div>
      </div>
    </div>
  ...
  root.querySelector('#btn-google').addEventListener('click', async () => {
    try {
      ...
      const message = friendlyLoginError(e, 'Google')
  ...
  root.querySelector('#btn-apple').addEventListener('click', async () => {
    try {
      ...
      const message = friendlyLoginError(e, 'Apple')
      showLoginError(message)
      showToast(message, 'error')
      const btn = root.querySelector('#btn-apple')
...
function friendlyLoginError(e, provider) {
  if (e?.code === 'auth/popup-blocked') {
    return '登入視窗被瀏覽器擋住。請允許彈出視窗，或用 Safari / Chrome 開啟 Logbook 後再登入。'
  }
  if (e?.code === 'auth/popup-closed-by-user') {
    return '登入尚未完成。請重新登入，並在跳出的視窗中完成驗證。'
  }
  if (e?.code === 'auth/operation-not-allowed' && provider === 'Apple') {
    return 'Apple Sign In 尚未完成設定，請先使用 Google Sign In。'
  }
  return e?.message || `${provider} 登入失敗，請稍後再試。`
}
